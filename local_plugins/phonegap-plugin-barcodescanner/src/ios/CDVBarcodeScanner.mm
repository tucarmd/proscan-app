...
- (void)barcodeScanCancelledAndEnterVin;
- (void)openDialog;
...
- (IBAction)enterVinButtonPressed:(id)sender;
...
- (void)barcodeScanCancelledAndEnterVin {
    [self barcodeScanDone:^{
        [self.plugin returnSuccess:@"enterVin" format:@"" cancelled:TRUE flipped:self.isFlipped callback:self.callback];
    }];
    if (self.isFlipped) {
        self.isFlipped = NO;
    }
...
- (IBAction)enterVinButtonPressed:(id)sender {
    [self.processor performSelector:@selector(barcodeScanCancelledAndEnterVin) withObject:nil afterDelay:0];
}
...
- (UIView*)buildOverlayView {
    ...
    id enterVinButton = [[[UIBarButtonItem alloc]
                          initWithTitle:@"Enter Manually"
                          style:UIBarButtonItemStyleDone
                          target:(id)self
                          action:@selector(enterVinButtonPressed:)
                          ] autorelease];
    ...
    if (_processor.isShowFlipCameraButton) {
        items = [NSMutableArray arrayWithObjects:flexSpace, cancelButton, flexSpace, enterVinButton, flexSpace, flipCamera, shutterButton, nil];
    } else {
        items = [NSMutableArray arrayWithObjects:flexSpace, cancelButton, flexSpace, enterVinButton, flexSpace, shutterButton, nil];
    }
#else
    if (_processor.isShowFlipCameraButton) {
        items = [@[flexSpace, cancelButton, flexSpace, enterVinButton, flexSpace, flipCamera] mutableCopy];
    } else {
        items = [@[flexSpace, cancelButton, flexSpace, enterVinButton, flexSpace] mutableCopy];
    }
#endif
    ...
    CGRect  rectArea       = CGRectMake(0, rootViewHeight - toolbarHeight - 20, rootViewWidth, toolbarHeight);
