...
|-Version     |   Date       |  Engineer   |        Note                     -|
  ...
  +V01.00.01    19Jan2011         HungVo      fixed livedata supported to 160 items
...
#ifdef _MSC_VER
typedef uint8 enumECUStatus;
#define eECU_Normal 0
#define eECU_Crashed 0x55
#define eECU_MAX 255
#else

typedef enum enumECUStatus
{
	eECU_Normal=0,
	eECU_Crashed=0x55,
	eECU_MAX=255
...
typedef struct _structOemProfile
{
    ...
    uint8               Reserve[_FLASH_BASE_SIZE_SMALL_-USED_OEM_BUFFER-OEM_INFO_LEN-4-1];
    enumECUStatus	eECUStatus;
...
#define gf_NotifyBusy()             gf_SYS_NotifyBusy(p_strtLinkProfile)
#ifndef DEBUG_LINE
#if DEBUG_LEVEL != 0
